...
bool SST::load(const std::string& model_path) {
    ...
    if (!SherpaOnnxFileExists(m_encoder_path.c_str())) {
        std::cerr << "SST: Encoder model not found: " << m_encoder_path << std::endl;
        return false;
    }
    std::cout << "SST: Encoder: " << m_encoder_path << std::endl;

    if (!SherpaOnnxFileExists(m_decoder_path.c_str())) {
        std::cerr << "SST: Decoder model not found: " << m_decoder_path << std::endl;
        return false;
    }
    ...
    config.model_config.provider = "cpu"; // Explicitly set provider to CPU
    ...
    if (!m_recognizer) {
        std::cerr << "SST: Failed to create SherpaOnnxOnlineRecognizer." << std::endl;
        return false;
    }
    ...
    m_audio_config.pUserData        = m_instance; // Pass the SST instance to the callback
...
void SST::data_callback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
    // Cast pUserData back to SST instance
    SST* sst_instance = static_cast<SST*>(pDevice->pUserData);

    if (sst_instance == nullptr || sst_instance->m_recognizer == nullptr || sst_instance->m_stream == nullptr) {
    ...
    SherpaOnnxOnlineStreamAcceptWaveform(sst_instance->m_stream, sst_instance->m_audio_config.sampleRate, float_samples.data(), frameCount);

    // Decode and get results
    if (SherpaOnnxIsOnlineStreamReady(sst_instance->m_recognizer, sst_instance->m_stream)) {
        SherpaOnnxDecodeOnlineStream(sst_instance->m_recognizer, sst_instance->m_stream);
        const SherpaOnnxOnlineRecognizerResult *result = SherpaOnnxGetOnlineStreamResult(sst_instance->m_recognizer, sst_instance->m_stream);
        if (result && result->text[0] != '\0') {
